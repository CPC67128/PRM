...
$("#archiveActionForm").submit(function () {
	confirmation = confirm('Etes vous sûr de vouloir archiver cette entreprise ?');
	if (!confirmation)
		return false;
...
&nbsp;
La suppression détruit définitivement les informations de cette entreprise, mais sans supprimer les contacts liés.
<div id="deleteActionFormResult"></div>
</form>

<script type="text/javascript" charset="utf-8">
$("#deleteActionForm").submit(function () {
	confirmation = confirm('Etes vous sûr de vouloir supprimer cette entreprise ?');
	if (!confirmation)
		return false;
...
<div class="leftLink" onclick="window.open('http://www.google.com/search?q=<?php echo GetCompanyGoogleItSearchString($row["company_id"]); ?>','Google it!','toolbar=1,scrollbars=1');">Google it!</div>