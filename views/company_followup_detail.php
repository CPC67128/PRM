...
for ($i = 0; $i < $n; $i++)
{
  ...
  <td>
  <span class="fa fa-trash-o fa-lg" onclick="DeleteCompanyNote(<?= $ligneNote["company_id"] ?>, <?= $ligneNote["note_id"] ?>);" />
  </td>
  </tr>
  <?php
}
...
function DeleteCompanyNote(contactId, noteId) {
	...
	if (confirmation) {
		$.post('../controllers/company_controller.php?type=remove_note&company_id='+contactId+'&note_id='+noteId,
			{ },
