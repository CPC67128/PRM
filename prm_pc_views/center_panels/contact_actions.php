...
$("#archiveActionForm").submit(function () {
	confirmation = confirm('<?php echo GetString('Etes vous sûr de vouloir archiver ce contact ?'); ?>');
	if (!confirmation)
		return false;
...
&nbsp;
<?php echo GetString('La suppression détruit définitivement les informations de ce contact.'); ?>
<div id="deleteActionFormResult"></div>
</form>

<script type="text/javascript" charset="utf-8">
$("#deleteActionForm").submit(function () {
	confirmation = confirm('<?php echo GetString('Etes vous sûr de vouloir supprimer ce contact ?'); ?>');
	if (!confirmation)
		return false;
